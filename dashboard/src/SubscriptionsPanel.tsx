...
export function SubscriptionsPanel() {
  ...
  const handleSearchManual = async () => {
    ...
    try {
      ...
      if (snap.empty) {
        ...
        // Pre-fill sub info
        const sub = (student as any).subscription || { type: 'none', allowedTeachers: [], allowedSubjects: [] };
        setSubType((sub.type === 'limited' || sub.type === 'full') ? sub.type : 'none');
        setSelectedTeachers(sub.allowedTeachers || []);
        setSelectedSubjects(sub.allowedSubjects || []);
        setSubEndDate(sub.endDate ? new Date(sub.endDate).toISOString().split('T')[0] : "");
  ...
  const handleSaveManualSub = async () => {
      ...
      <div className="panel-header">
        ...
        <div style={{ display: "flex", gap: "10px" }}>
          ...
          <button 
             ...
             style={{ background: "var(--gold)", color: "white", padding: "6px 14px", borderRadius: "8px", border: "none", cursor: "pointer", fontWeight: "bold", fontSize: "0.85rem" }}
          >
            منح اشتراك يدوي
          </button>
          <button 
                ...
                <tr key={student.id}>
                  ...
                  <td>
                    {sub.type === 'limited' ? (
                       ...
                       <span style={{ color: "var(--emerald-main-main)", fontWeight: "bold", background: "var(--emerald-main-lightest)", padding: "4px 10px", borderRadius: "12px", fontSize: "0.8rem" }}>شامل</span>
                    ) : (
              ...
              <div style={{ padding: "24px" }}>
                <div style={{ marginBottom: "20px", display: "flex", gap: "8px", flexWrap: "wrap" }}>
                  <button
                    ...
                    style={{ flex: 1, padding: "10px", borderRadius: "12px", border: subType === "full" ? "2px solid var(--emerald-main-main)" : "1px solid var(--border-light)", background: subType === "full" ? "var(--emerald-main-lightest)" : "white", fontWeight: "bold", color: subType === "full" ? "var(--emerald-main-main)" : "var(--text-secondary)", cursor: "pointer" }}
                  >
                    اشتراك شامل
                  </button>
                  <button
        ...
        {isManualModalOpen && (
          ...
          >
            <motion.div
              initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }}
              onClick={e => e.stopPropagation()}
              style={{ background: "white", borderRadius: "16px", width: "100%", maxWidth: "550px", display: "flex", flexDirection: "column", boxSizing: "border-box", margin: "auto 0" }}
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "20px 24px", borderBottom: "1px solid var(--border-light)", flexShrink: 0 }}>
                <h3 style={{ margin: 0, color: "var(--text-primary)", fontSize: "1.1rem" }}>منح اشتراك طالب يدوياً</h3>
              ...
              <div style={{ padding: "24px" }}>
                ...
                {manualFoundStudent && (
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                    ...
                    <div style={{ marginBottom: "20px" }}>
                      <label style={{ display: "block", marginBottom: "8px", fontWeight: "bold" }}>تاريخ انتهاء الاشتراك</label>
                      <input
                        type="date"
                        value={subEndDate}
                        onChange={(e) => setSubEndDate(e.target.value)}
                    ...
                    <div style={{ marginBottom: "20px" }}>
                      <label style={{ display: "block", marginBottom: "8px", fontWeight: "bold" }}>التكلفة (IQD)</label>
                      <input
                        type="number"
                        value={manualCost}
                    ...
                    {subType === "limited" && (
                      <div style={{ display: "flex", flexDirection: "column", gap: "20px", marginBottom: "20px" }}>
                        <div>
                          <h4 style={{ marginBottom: "10px" }}>المعلمين المسموحين</h4>
                          <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
                            {teachers.map(t => {
                              const isSelected = selectedTeachers.includes(t.id);
                              return (
                                <div
                                  key={t.id}
                                  onClick={() => toggleTeacher(t.id)}
                                  style={{ padding: "6px 12px", borderRadius: "20px", border: "1px solid", borderColor: isSelected ? "var(--gold)" : "var(--border-light)", background: isSelected ? "var(--gold)" : "white", color: isSelected ? "var(--emerald-main-main)" : "var(--text-primary)", cursor: "pointer", fontSize: "0.85rem", fontWeight: "bold", display: "flex", alignItems: "center", gap: "4px" }}
                                >
                                  {isSelected && <Check size={12} color="var(--emerald-main-main)" />} {t.name}
                                </div>
                              );
                        ...
                        <div>
                          <h4 style={{ marginBottom: "10px" }}>المواد المسموحة</h4>
                          <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
                            {IRAQI_SUBJECTS.map(subName => {
                              const isSelected = selectedSubjects.includes(subName);
                              return (
                    ...
                    >
                      {isSaving ? "جاري الحفظ..." : "تأكيد ومنح الاشتراك"}
                    </button>
                  </motion.div>
                )}
              </div>
