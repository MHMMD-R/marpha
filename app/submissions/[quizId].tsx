...
import { ActivityIndicator, Alert as NativeAlert, FlatList, Image, KeyboardAvoidingView, Modal, Platform, ScrollView, StyleSheet, Text, TextInput, TouchableOpacity, View, Keyboard } from 'react-native';
...
export default function QuizSubmissionsScreen() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (error) => {
        ...
        setLoading(false);
        NativeAlert.alert('خطأ', 'تعذر تحميل التسليمات.');
      }
    );
    return () => unsubscribe();
  }, [quizId]);
  ...
  const handleSaveGrade = async () => {
    ...
    if (!gradeInput.trim()) return NativeAlert.alert('تنبيه', 'الرجاء إدخال الدرجة أولاً');
    ...
    try {
      ...
      if (selectedSubmission.studentId) {
        ...
        if (studentDoc.exists()) {
          ...
          const expoPushToken =
            studentData.notificationsEnabled === false ? null : studentData.expoPushToken;
          if (expoPushToken) {
            const messages = [{
              to: expoPushToken,
              sound: 'default',
              ...
              data: { route: 'quiz', quizId: quizId as string },
            }];
      ...
      NativeAlert.alert('خطأ', error.message || 'حدث خطأ أثناء حفظ الدرجة.');
    } finally {
