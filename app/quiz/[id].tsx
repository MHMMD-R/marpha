...
export default function TakeQuizScreen() {
  usePreventScreenCapture('take-quiz');
  ...
  const [closureReason, setClosureReason] = useState<'deadline' | 'manual' | ''>('');
      ...
      try {
        ...
        if (docSnap.exists()) { setQuiz({ id: docSnap.id, ...docSnap.data() });
          ...
          if (quizData.status === 'finished') {
            setIsClosed(true);
            setClosureReason('manual');
          } else if (quizData.deadline) {
            const deadline = quizData.deadline.toDate();
            if (new Date() > deadline) {
              setIsClosed(true);
              setClosureReason('deadline');
            }
          }
        } else {
  ...
  const uploadImage = async (uri: string, idx: number) => {
    ...
    return uploadR2File({
      ...
      mimeType: 'image/jpeg',
      bucketType: 'QUIZZES',
      folder: `quiz_answers_${userUid}`,
      fallbackFileName: fileName,
      errorLabel: 'رفع صورة الإجابة',
    });
  };
  ...
  const pickImage = async (qIdx: number) => {
    ...
    try {
      ...
      console.error('Quiz answer image picker error:', error);
      Alert.alert('خطأ', 'تعذر اختيار صورة الإجابة.');
    }
  };
          ...
          <View style={styles.content}>
            <ScrollView contentContainerStyle={{ padding: 24, flexGrow: 1, justifyContent: 'center' }}>
              <View style={styles.resultCard}>
                ...
                <Text style={styles.resultSubtext}>لقد قمت بتسليم هذا الاختبار بالفعل. شكراً لك!</Text>
                ...
                <TouchableOpacity style={styles.resultBackBtn} onPress={() => router.back()} activeOpacity={0.8}>
      ...
      <View style={styles.wrapper}>
        ...
        <SafeAreaView style={{ flex: 1 }} edges={['top', 'bottom']}>
          <View style={styles.header}>
            <BackButton />
            <View style={styles.headerTitleContainer}>
              <Text style={styles.headerSubtitle}>انتهى الوقت</Text>
              <Text style={styles.headerTitle} numberOfLines={1}>{quiz?.title || 'الاختبار'}</Text>
            </View>
          </View>
          ...
          <View style={styles.content}>
            <ScrollView contentContainerStyle={{ padding: 24, flexGrow: 1, justifyContent: 'center' }}>
              <View style={styles.resultCard}>
                <View style={[styles.resultIconCircle, { backgroundColor: '#FFF0F0' }]}>
                  <Ionicons name="lock-closed" size={56} color={C.danger} />
                </View>

                <Text style={styles.resultTitle}>الاختبار مغلق</Text>
                <Text style={styles.resultSubtext}>
                  {closureReason === 'deadline' 
                    ? 'عذراً، لقد انتهى الوقت المحدد لهذا الاختبار ولا يمكن استقبال إجابات جديدة.'
                    : 'لقد قام المعلم بإغلاق هذا الاختبار حالياً.'}
                </Text>

                <TouchableOpacity
                  style={[styles.resultBackBtn, { backgroundColor: C.danger }]}
                  onPress={() => router.back()}
                  activeOpacity={0.8}
                >
                  <Ionicons name="arrow-back" size={18} color={C.white} />
