  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      const syncVisibleUnread = () => {
        if (!teacherLoaded || !studentsLoaded) {
          setTotalUnread(0);
          return;
        }

        const visibleStudentIds = new Set(
          teacherProfile ? filterStudentsForTeacher(allStudents, teacherProfile).map(student => student.id) : [],
        );
        let count = 0;
        chatDocs.forEach((chatData) => {
          ...
          if (!otherParticipant || !visibleStudentIds.has(otherParticipant)) return;
          count += chatData[`unreadCount_${user.uid}`] || 0;
        });
        setTotalUnread(count);
      };

      const unsubscribeTeacher = onSnapshot(doc(db, 'teachers', user.uid), (snap) => {
        teacherLoaded = true;
        teacherProfile = snap.exists() ? { id: snap.id, uid: snap.id, ...snap.data() } : null;
        syncVisibleUnread();
      });

      const unsubscribeStudents = onSnapshot(collection(db, 'students'), (snapshot) => {
        studentsLoaded = true;
        allStudents = snapshot.docs.map(docSnap => ({ id: docSnap.id, uid: docSnap.id, ...docSnap.data() }));
        syncVisibleUnread();
      });
