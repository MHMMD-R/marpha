...
import { ActivityIndicator, Alert as NativeAlert, Animated, Dimensions, Easing, FlatList, Image, KeyboardAvoidingView, Modal, Platform, ScrollView, StyleSheet, Text, TextInput, TouchableOpacity, View, Keyboard } from 'react-native';
          ...
          <View style={styles.bannerContent}>
            ...
            <View style={styles.bannerTitleRow}>
              <View style={styles.bannerTitleCol}>
                ...
                <View style={[styles.statusPill, item.status === 'finished' && { backgroundColor: 'rgba(255,59,48,0.15)' }]}>
                  <View style={[styles.statusDot, item.status === 'finished' && { backgroundColor: C.danger }]} />
                  <Text style={styles.statusText}>{item.status === 'finished' ? 'منتهي' : 'نشط'}</Text>
                </View>
              </View>
          ...
          {item.deadline && (
            <View style={styles.deadlineBanner}>
              ...
              <Text style={styles.deadlineText}>
                ينتهي في: {item.deadline.toDate().toLocaleString('ar-EG', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
              </Text>
            </View>
          )}
          <View style={styles.cardBodyRow}>
            ...
            <View style={styles.cardActionsRow}>
              {item.status !== 'finished' && (
                <TouchableOpacity 
                  onPress={() => {
                    Alert.alert('إنهاء الاختبار', 'هل أنت متأكد من رغبتك في إنهاء هذا الاختبار الآن؟ لن يتمكن الطلاب من تقديم إجابات جديدة.', [
                      { text: 'إلغاء', style: 'cancel' },
                      { text: 'إنهاء الآن', style: 'destructive', onPress: async () => {
                        await updateDoc(doc(db, 'quizzes', item.id), { status: 'finished' });
                      }}
                    ]);
                  }}
                  style={styles.finishChip}
                >
                  <Text style={styles.finishChipText}>إنهاء الآن</Text>
                  <Ionicons name="stop-circle-outline" size={14} color={C.danger} />
                </TouchableOpacity>
              )}
...
export default function TeacherQuizzesScreen() {
  ...
  const uploadImage = async (uri: string, prefix: string, userUid: string) => {
    ...
    return uploadR2File({
      ...
      mimeType: 'image/jpeg',
      bucketType: 'QUIZZES',
      folder: `quizzes_${userUid}`,
      fallbackFileName: fileName,
      errorLabel: 'رفع صورة السؤال',
    });
  };

  const handleAddQuiz = async () => {
    if (isSubmitting) return;
    if (!quizTitle.trim()) return NativeAlert.alert('تنبيه', 'الرجاء إدخال عنوان الاختبار على الأقل');
    if (questions.length === 0) return NativeAlert.alert('تنبيه', 'الرجاء إضافة سؤال واحد على الأقل');
    const user = auth.currentUser;
    if (!user) return NativeAlert.alert('خطأ', 'يجب تسجيل الدخول قبل إنشاء الاختبار.');

    for (const q of questions) {
      if (!q.qUri) return NativeAlert.alert('تنبيه', 'الرجاء التأكد من إضافة صورة السؤال لجميع الأسئلة');
    }
    ...
    try {
      ...
      const [studentsSnap, teacherSnap] = await Promise.all([
        getDocs(collection(db, 'students')),
        getDoc(doc(db, 'teachers', user.uid)),
      ]);
      const teacherProfile = { id: user.uid, uid: user.uid, ...(teacherSnap.exists() ? teacherSnap.data() : {}) };
      const allowedStudents = filterStudentsForTeacher(
        studentsSnap.docs.map((docSnap) => ({ id: docSnap.id, uid: docSnap.id, ...docSnap.data() })),
        teacherProfile
      );
      const targetTokens = Array.from(new Set(
        allowedStudents
          .filter((student: any) => student.notificationsEnabled !== false)
          .map((student: any) => student.expoPushToken)
          .filter(Boolean)
      ));
      ...
      setQuizTitle(''); setQuestions([]); setDeadline(null);
    } catch (error: any) {
      NativeAlert.alert('خطأ', error.message || 'حدث خطأ أثناء إنشاء الاختبار.');
    } finally {
  ...
  const pickImage = async (id: string, type: 'qUri') => {
    ...
    try {
      ...
      NativeAlert.alert('خطأ', 'تعذر اختيار صورة السؤال.');
    }
  };
              ...
              <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
                ...
                <View style={styles.inputGroup}>
                  <View style={styles.inputLabelRow}>
                    ...
                    <Text style={styles.inputLabel}>تحديد موعد انتهاء (اختياري)</Text>
                  </View>
                  <TouchableOpacity 
                  ...
                  >
                    <Text style={[styles.deadlineSelectorText, !deadline && { color: C.textSecondary }]}>
                      {deadline 
                        ? deadline.toLocaleString('ar-EG', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' })
                        : 'لم يتم تحديد موعد'}
                    </Text>
                    {deadline && (
                      <TouchableOpacity 
...
const cardStyles = StyleSheet.create({
  ...
  deadlineSelectorText: { fontSize: 14, color: C.textPrimary, fontWeight: '600' },
  deadlineClearBtn: { padding: 2 },
