...
function getArabicErrorMessage(error: any): string {
  ...
  switch (code) {
    case "auth/email-already-in-use":
      return "اسم المستخدم هذا مستخدم بالفعل. يرجى اختيار اسم مستخدم آخر.";
    case "auth/invalid-email":
      return "اسم المستخدم يحتوي على أحرف غير مسموح بها.";
    case "auth/weak-password":
      return "كلمة المرور ضعيفة جداً. يجب أن تكون 6 أحرف على الأقل.";
    case "auth/user-not-found":
      return "لا يوجد حساب بهذا الاسم. تأكد من اسم المستخدم أو أنشئ حساباً جديداً.";
    ...
    case "auth/network-request-failed":
      return "فشل الاتصال بالإنترنت. تأكد من اتصالك بالشبكة وحاول مرة أخرى.";
    case "auth/user-disabled":
      return "تم تعطيل هذا الحساب. يرجى التواصل مع الإدارة.";
    case "auth/requires-recent-login":
      return "يرجى تسجيل الدخول مرة أخرى لإتمام هذه العملية.";
    case "auth/operation-not-allowed":
      return "هذه العملية غير مسموحة حالياً. يرجى التواصل مع الدعم.";
    default:
...
export default function LoginScreen() {
  ...
  useEffect(() => {
    const checkLockout = async () => {
      const stored = await AsyncStorage.getItem('app_loginLockoutUntil');
      if (stored) {
  ...
  const handleBarcodeScanned = async ({ data }: { data: string }) => {
    ...
    try {
      ...
      Alert.alert('فشل تسجيل الدخول', getArabicErrorMessage(err));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleAuth = async () => {
    ...
    try {
      ...
      if (isLogin) {
        ...
        if (teacherDoc.exists()) {
          ...
          if (studentDoc.exists()) {
            ...
            const boundDeviceId = studentData.boundDeviceId;

            if (!boundDeviceId || boundDeviceId === deviceId) {
              // Bind or Update session
              await updateDoc(doc(db, 'students', user.uid), {
                boundDeviceId: deviceId,
                boundDeviceOS: Platform.OS,
                boundAt: studentData.boundAt || new Date().toISOString(),
                activeSessionId: Math.random().toString(36).substring(2)
              });
              
              if (studentData.isSetupComplete === false) {
                router.replace('/setup');
              } else {
                router.replace('/(tabs)');
              }
            } else {
              // Device mismatch - check cooldown
              const lastChange = studentData.lastDeviceChangeAt ? new Date(studentData.lastDeviceChangeAt) : null;
              const now = new Date();
              const diffDays = lastChange ? (now.getTime() - lastChange.getTime()) / (1000 * 3600 * 24) : 999;

              if (diffDays >= 30) {
                Alert.alert(
                  'تنبيه الأمان',
                  'هذا الحساب مرتبط بجهاز آخر. هل تريد نقل الحساب إلى هذا الجهاز؟ (يمكنك فعل ذلك مرة واحدة كل 30 يوماً)',
                  [
                    { text: 'إلغاء', onPress: () => auth.signOut(), style: 'cancel' },
                    { text: 'نقل الحساب', onPress: async () => {
                        try {
                          await updateDoc(doc(db, 'students', user.uid), {
                            boundDeviceId: deviceId,
                            boundDeviceOS: Platform.OS,
                            boundAt: new Date().toISOString(),
                            lastDeviceChangeAt: new Date().toISOString(),
                            deviceChangeCount: (studentData.deviceChangeCount || 0) + 1,
                            activeSessionId: Math.random().toString(36).substring(2)
                          });
                          router.replace(studentData.isSetupComplete ? '/(tabs)' : '/setup');
                        } catch (e: any) {
                          Alert.alert('خطأ', getArabicErrorMessage(e));
                        }
                      } 
                    }
                  ]
                ...
                Alert.alert('تنبيه', `هذا الحساب مرتبط بجهاز آخر. يمكنك نقل الحساب مجدداً بعد ${remainingDays} يوماً.`);
                await auth.signOut();
              }
            }
          }
        ...
        await AsyncStorage.removeItem('app_loginAttempts');
        await AsyncStorage.removeItem('app_loginLockoutUntil');
      ...
      const arabicMsg = getArabicErrorMessage(error);
      if (isLogin) {
        const attemptsStr = await AsyncStorage.getItem('app_loginAttempts');
        const currentAttempts = parseInt(attemptsStr || '0', 10) + 1;
        await AsyncStorage.setItem('app_loginAttempts', currentAttempts.toString());
        
        if (currentAttempts >= 5) {
          const newLockoutTime = Date.now() + 5 * 60 * 1000;
          setLockoutUntil(newLockoutTime);
          await AsyncStorage.setItem('app_loginLockoutUntil', newLockoutTime.toString());
          Alert.alert('حظر الدخول', 'تم حظر الدخول لكثرة المحاولات الخاطئة. يرجى المحاولة بعد 5 دقائق.');
        } else {
          Alert.alert('فشل تسجيل الدخول', `${arabicMsg}\n\nالمحاولة ${currentAttempts} من 5`);
        }
      } else {
        Alert.alert('فشل إنشاء الحساب', arabicMsg);
      }
    } finally {
      setIsLoading(false);
    }
